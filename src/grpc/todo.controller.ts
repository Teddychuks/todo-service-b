...
export class TodoGrpcController {
  ...
  async createTodo(data: CreateTodoRequest): Promise<TodoResponse> {
    this.logger.log(`Received CreateTodo gRPC request from Service A: ${JSON.stringify(data)}`);
  ...
  async getTodoById(data: TodoById): Promise<TodoResponse> {
    this.logger.log(`Received GetTodoById gRPC request from Service A for ID: ${data.id}`);
  ...
  async updateTodo(data: UpdateTodoRequest): Promise<TodoResponse> {
    this.logger.log(`Received UpdateTodo gRPC request from Service A: ${JSON.stringify(data)}`);
  ...
  async deleteTodo(data: TodoById): Promise<DeleteResponse> {
    this.logger.log(`Received DeleteTodo gRPC request from Service A for ID: ${data.id}`);
